...
template <class T> const T LinkedList<T>::PopFront()
{
    ...
    //If the head and the tail are the same pointer (i.e. exactly 1 element in our linked list as we previously checked if its empty) then we need to update the tail as well as the head 
    if(head_ == tail_)
    {
        //update tail since we update head regardless just after this if condition 
        tail_ = head_->GetNext(); 
    }
...
template <class T> void LinkedList<T>::PushBack(T value)
{
    ...
    {
        head_ = node; 
        //update tail as the tail has now changed (otherwise would point to the previous tail before we pushed something to the back and that's no good!)
        tail_ = node; 
    ...
    //Update tail pointer to this new node which is the new tail. 
    tail_ = node; 
...
template <class T> const T LinkedList<T>::PopBack()
{
    ...
    ListElement<T> *secondLastElem = nullptr; 
    ...
    else
    {
        ...
        //Tail should point to the last element now 
        tail_ = lastElem;
    }
...
template <class T>const T LinkedList<T>::Back()
{
    ...
    //Return the value held in the tail pointer 
...
template <class T> void LinkedList<T>::Insert(int index, T value) 
{
    ...
    if(previous == nullptr )
    {
        ...
        node->SetNext(head_);
        //Update tail pointer as the tail has changed. 
        tail_ = head_; 
    ...
    else
    {
        ...
        //If the previous is the tail then we know we need to update the tail to the current 
        if(previous == tail_)
        {
            //This is the case where the place we want to add our new element at the tail index, which would push the tail index 1 right I think 
            tail_ = node; 

            //This should handle the case where we want to insert at the index in which the tail was residing, this would push the tail right and thus we would need to update the tail to the new tail which should have it's next pointer as null always 
...
template <class T> void LinkedList<T>::Erase(int index) 
{
    ...
    for (i = 0; i < index && current; ++i)
    {
        ...
        //Case where if current is our tail 
        if(current == tail_ && i = index)
        {
            //Then our tail should be previous as this will be removed 
            tail_ = previous; 
        }
...
template <class T> void LinkedList<T>::Reverse() 
{
  ...
  while(current != nullptr )
  {
    ...
    //Temp variable to hold head or tail so we can swap head and tail :) 
    ListElement <T> *temp = head_; 
    head_ = tail_; 
    tail_ = temp; 
    //If for some reason the tail pointer was not updated to point to null after then we need to fix this or we would run into circular/infinite recursion issues 
    if(tail_->GetNext() != nullptr)
    {
        tail_->SetNext(nullptr); 
    }

    //TODO: CHECK POINTER TO POINTER LOGIC TO MAKE SURE THIS ACTUALLY WORKS 
    //std::swap(head_,tail_); //This is probably the easiest way of doing it 
...
template <class T> void LinkedList<T>::RemoveValue(T value) 
{
    ...
    while(current)
    {
        if(current->GetValue() == value)
        {
            ...
            {
                //Handle case where the current could be the tail and since it is the value we want to remove we must update the tail to the previous 
                if(current == tail_)
                {
                    tail_ = previous; 
                }
